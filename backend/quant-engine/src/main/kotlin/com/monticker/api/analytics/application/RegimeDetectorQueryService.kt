...
class RegimeDetectorQueryService(
    private val jdbc: JdbcTemplate,
) {
    @Cacheable(cacheNames = [CacheConfig.REGIME], key = "'stock:' + #stockId")
    fun classifyRegimeCompute(stockId: Long): RegimeResult {
        ...
        return computeRegime(candles).copy(detectedAt = candles.last().date)
    }

    @Cacheable(cacheNames = [CacheConfig.REGIME], key = "'market:' + #market")
    fun classifyMarketRegimeCompute(market: String): RegimeResult {
    ...
    fun calculateADX(candles: List<DailyCandle>, period: Int = 14): Double {
        ...
        for (i in (period + 1) until candles.size) {
            ...
            dxValues.add(if (sum != 0.0) abs(plusDI - minusDI) / sum * 100 else 0.0)
        }
    ...
    private fun rollingVolatilitySeries(candles: List<DailyCandle>, windowPeriod: Int = 20): List<Double> {
        ...
        for (end in windowPeriod until candles.size) {
            series.add(calculateVolatility(candles.subList(end - windowPeriod, end + 1), windowPeriod))
        }
        return series
    }
