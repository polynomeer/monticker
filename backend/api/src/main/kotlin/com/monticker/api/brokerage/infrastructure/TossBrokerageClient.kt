...
class TossBrokerageClient(
    ...
    override fun getBalance(credentials: BrokerageCredentials): BrokerageBalance {
        return try {
            ...
            log.warn("[CircuitBreaker:toss] 요청 차단됨 — 잔고 조회 불가")
            throw ExternalServiceUnavailableException("toss", "Toss API 장애로 서킷브레이커가 열려 있습니다. 잔고를 확인할 수 없습니다.", e)
        } catch (e: RestClientException) {
            log.error("[Toss] 잔고 조회 실패: {}", e.message)
            throw ExternalServiceUnavailableException("toss", "Toss 잔고 조회에 실패했습니다. 잠시 후 다시 시도하세요.", e)
        }
    }
