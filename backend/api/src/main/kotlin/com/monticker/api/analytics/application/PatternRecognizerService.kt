...
data class PatternMatch(
    ...
    val candleFrom: LocalDate,
    val candleTo: LocalDate,
...
class PatternRecognizerService(
    ...
    fun zigZag(candles: List<DailyCandle>, thresholdPct: Double = 3.0): List<SwingPoint> {
        ...
        var direction: SwingType? = null // direction of current trend: HIGH means we're tracking a rising run looking for top
        var extremeIndex = 0
    ...
    fun detectDoubleBottom(swings: List<SwingPoint>): PatternMatch? {
        ...
        if (riseFromLows < 5.0) return null
    ...
    fun detectDoubleTop(swings: List<SwingPoint>): PatternMatch? {
        ...
        if (diffPct > 2.0) return null
        val dropFromHighs = (minOf(high1, high2) - low) / minOf(high1, high2) * 100
        if (dropFromHighs < 5.0) return null
    ...
    fun detectHeadAndShoulders(swings: List<SwingPoint>): PatternMatch? {
        ...
        val types = last5.map { it.type }
        if (types != listOf(SwingType.HIGH, SwingType.LOW, SwingType.HIGH, SwingType.LOW, SwingType.HIGH)) return null
        ...
        val confidence = (100 - shoulderDiffPct * 10).coerceIn(0.0, 100.0).toInt()
        return PatternMatch(
    ...
    fun detectAscendingTriangle(swings: List<SwingPoint>): PatternMatch? {
        ...
        val ascending = lowValues.zipWithNext().all { (a, b) -> b > a }
        if (!ascending) return null

        val confidence = (100 - highDiffPct * 10).coerceIn(0.0, 100.0).toInt()
        return PatternMatch(
    ...
    fun detectDescendingTriangle(swings: List<SwingPoint>): PatternMatch? {
        ...
        val highValues = highs.map { it.price.toDouble() }
        val descending = highValues.zipWithNext().all { (a, b) -> b < a }
        if (!descending) return null

        val confidence = (100 - lowDiffPct * 10).coerceIn(0.0, 100.0).toInt()
        return PatternMatch(
