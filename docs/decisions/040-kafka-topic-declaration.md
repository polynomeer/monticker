...
**Note (2026-09-11):** §3의 파티션 산정 근거 "파티션당 안전 처리량 ~2,000 msg/s"를 L-03 부하 기준선으로
실측했다 — **현재 워커의 단일 파티션 처리 상한은 ~600 tick/s**다(3배 낙관적이었다). 유입 2,000/s에서
랙이 선형으로 쌓이고 소진되지 않는다. 원인은 틱마다 동기로 하는 DB 작업(특히 ADR-044가 제거할 알림 룰
조회). **추가 (같은 날 저녁):** [ADR-046](046-detector-state-in-memory.md)(감지기 상태 메모리화) 후 단일 스레드 ~2,000 tick/s로 회복돼 원래 가정이 맞는 숫자가 됐다. 아래 문장은 ADR-046 이전 기준이다. 따라서 (1) T2 목표 파티션 256은 유지하되 그 근거는 "600/s × 256 ≈ 150k/s"로 바뀌고,
(2) ADR-044가 파티션 증설보다 먼저다 — 컨슈머당 처리량을 올리지 않으면 파티션 수만큼 컨슈머 스레드가
필요하다. 결과: [resilience-plan §5.5](../resilience-plan.md).
