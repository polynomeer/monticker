...
- **신청 절차를 실제로 확인해보니 KIS엔 없는 제약이 하나 더 있었다** — Toss는 client_id/client_secret 발급과 별도로 **호출 서버의 공인 IP를 WTS "허용 IP 관리"에 등록**해야 하며, 등록 안 된 IP의 호출은 403으로 막힌다(`platform-api-keys.md` §2.3). `backend/worker`를 배포할 서버의 아웃바운드 IP가 고정돼 있지 않으면(동적 NAT 등) 이 코드는 처음부터 작동하지 않는다 — 코드 문제가 아니라 인프라 사전조건이라는 걸 다음에 이 경로를 디버깅할 사람이 먼저 알아야 한다.
