...
> **Note (2026-09-14)**: `trading-service`·`quant-engine` 모두 api가 실제로 위임을 연결한 적이 없어
> [ADR-048](048-retire-trading-service.md)·[ADR-049](049-retire-quant-engine.md)로 폐기됐다. 이 ADR이 다룬 "api → 내부
> 서비스" 위임 대상은 지금 없다. K8s DNS 탐색과 nginx 게이트웨이 자체는 인프라 컴포넌트·web·api에 그대로 유효하다.
