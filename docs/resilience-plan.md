...
| **L-06** | `backtest-flood` | k6 | 동시 100건 제출 | 429로 거절, **주문 경로 p95 무영향** — **실행됨** (2026-09-14, `bench/scenarios/backtest-bulkhead.js`): 조회 4종 p95 대조군 7.7ms → 폭주 20 VU 7.1ms / 80 VU 29.4ms(429 11.6k/s), bulkhead PASS. [ADR-049](decisions/049-retire-quant-engine.md) 근거 |
...
### 5.5 기준선 측정 결과 (2026-09-11, 로컬 — 상대 비교용)

환경: MacBook 한 대에서 k6 + api + worker + Kafka(단일 브로커, `market.ticks` 파티션 1) + Postgres/Redis/ES
전부 실행. **절대값이 아니라 ADR-038/039/040 적용 전후를 비교할 "before" 수치다.**
스크립트: `bench/scenarios/rest.js`, `ws-fanout.js`, `tick-storm.sh`. 결과 JSON: `bench/results/latest_*`.

#### L-01 rest-baseline (10 → 50 → 100 VU, 조회 5종 + 인증 거래 경로 3종)

| 지표 | 값 | SLO(§4.3) |
|------|-----|----------|
| 처리량 | **326 req/s** | — |
| p50 / p90 / p95 | 8.7 / 36 / **52 ms** | p95 < 200 ✅ |
| 실패율 | 0.02% (전부 주문 429 = userId 레이트리밋 정상 동작) | < 0.1% ✅ |
| 주문 경로 5xx | 0 | 0 ✅ |
| 가장 느린 경로 | 지갑 p95 71ms, 검색 p95 70ms | — |

100 VU에서 여유가 크다. REST는 지금 병목이 아니다.

#### L-02 ws-fanout (50 → 200 → 500 연결, 연결당 20종목 구독, 틱 202/s)

| | **(b) 현재 프론트 동작** — `/topic/market` 구독 포함 | **(a) 종목별 구독만** — ADR-039 적용 후 근사 |
|---|---|---|
| 총 메시지 | **49,998 msg/s** (그중 **91%가 `/topic/market`**) | 4,795 msg/s |
| 연결당 | **199 msg/s** | 19 msg/s (= 구독 종목 수, 정확히 기대값) |
| e2e 지연 p50 | **1.5 s** | 41 ms |
| e2e 지연 p95 | **7.6 s** | 113 ms |
| e2e 지연 max | **13.6 s** | 846 ms |
| SLO p99 < 300ms | ❌ (25배 초과) | ❌ (max 기준; p99는 다음 실행부터 기록) |

**적용 후 (2026-09-11, ADR-038 conflation + ADR-039 전역 토픽 제거, 같은 곡선)**

| | before (b) 현재 프론트 | before (a) 종목별만 | **after** |
|---|---|---|---|
| 연결당 msg/s | 199 | 19 | 19 |
| e2e p50 | 1,529 ms | 41 ms | 84 ms |
| e2e p95 | 7,645 ms | 113 ms | **139 ms** |
| e2e p99 | — | — | **159 ms** ✅ SLO 300 |
| e2e max | 13,639 ms | 846 ms | **179 ms** |

conflation은 중앙값을 41→84ms로 올린다(최대 100ms 버퍼링 — 설계대로) 대신 **꼬리를 846→179ms로
잘랐다.** SLO 판정은 p99라 이 교환이 맞다. 같은 실행에서 api 인스턴스 2개(파티션 4개)가 각각
200종목 중 196개의 틱을 받았다 — 컨슈머 그룹 분할이 남아 있었다면 인스턴스당 ~50종목이었다
(ADR-038 정확성 판정 ✅). 결과: `bench/results/latest_ws-after-adr038-039.json`.

**500 연결에서 적용 전 구조는 시세를 평균 1.5초, 최악 13.6초 늦게 보여줬다.** 원인의 91%는
[ADR-039](decisions/039-drop-global-market-topic.md)가 폐지하기로 한 전역 토픽이다 — 그것만 빼도
지연이 **67배** 줄고 메시지가 10배 준다. 남는 113ms→846ms 꼬리는 [ADR-038](decisions/038-broadcast-consumer-partition-assignment.md)의
conflation 대상이다. **이 두 수치가 ADR-038/039의 완료 판정 기준선이다.**

#### L-03 tick-storm (Go 게이트웨이, 202종목, 단계별 60초)

| 유입 | 워커 처리 | 컨슈머 랙 (60s 후) | 파이프라인 p99 | 판정 |
|------|----------|------------------|-------------|------|
| ~200 tick/s | 200/s | 0 | 1.2 s | 따라온다 |
| ~2,000 tick/s | **~600/s** | **80,800 ↑** (선형 증가, 소진 안 됨) | 43 s | **못 따라온다** |
| ~10,000 tick/s | ~400/s | 530,000 ↑ | 103 s | 붕괴 |

**적용 후 (2026-09-11)** — 순서대로 적용하며 같은 곡선으로 재측정:

| 단계 | 2,000/s 유입 시 처리 | 10,000/s 유입 시 처리 | 비고 |
|------|------------------|-------------------|------|
| 기준선 | ~600/s, 랙 선형 증가 | ~400/s | 파티션 1 |
| + ADR-044 (룰 인덱스) | **~600/s** — 변화 없음 | ~400/s | 가설이 이 규모에선 틀렸다 |
| + ADR-046 (감지기 상태 메모리화) | **~2,000/s, 랙 0, p99 200ms** | ~1,800/s | 스택 샘플링이 가리킨 곳 |
| + 컨슈머 동시성 4 | — | **~5,000/s** | 12 파티션 중 4 스레드, 노트북 CPU 한계 |

기준선 대비 **8배**. 과정에서 배운 것:
- **ADR-044의 가설("틱당 룰 DB 조회가 주원인")은 룰 10개 규모에서 틀렸다.** 코드를 읽고 세운 가설이었다.
  적용 후 처리량이 그대로여서 부하 중 컨슈머 스레드 스택을 15회 샘플링했더니 13회가 감지기의 동기 Redis
  대기였다 — **틱당 Redis 왕복 7회**(시세 1 + 가격 감지기 4 + 거래량 감지기 2) ≈ 1.4ms = 700/s.
  [ADR-046](decisions/046-detector-state-in-memory.md). ADR-044는 T2 룰 규모(수백만)에서 필요하지만
  지금 숫자에는 영향이 없다. **성능 작업은 프로파일 없이 하면 안 된다.**
- **재측정이 결함 2개를 더 찾았다**: (1) Go 게이트웨이의 kafka-go `BatchTimeout` 1초 — 파티션이 여러 개가
  되자 부분 배치가 매번 1초를 기다려 종목당 1 msg/s로 붕괴(`e414da9`), ADR-040 파티션 증설 직후 운영에서
  터졌을 것. (2) `LatencyTracker`가 틱마다 WARN — 백로그에서 60초에 133,749줄, 처리량 절반으로 떨어지는
  자기강화 루프(`23b0bec`).
- ADR-040의 "파티션당 ~2,000 msg/s"는 **ADR-046 이후에는 맞는 숫자**가 됐다.

**기준선의 단일 파티션 워커 처리 상한 ≈ 600 tick/s.** [ADR-040](decisions/040-kafka-topic-declaration.md)이
파티션 수 산정에 쓴 "파티션당 ~2,000 msg/s"는 **3배 낙관적이었다** — ADR이 "실측으로 교체한다"고
예고한 그 숫자다. 원인은 틱마다 동기로 하는 일의 양: Redis 쓰기 + 캔들 집계 + 이벤트 감지 +
**알림 룰 DB 조회**([ADR-044](decisions/044-alert-rule-in-memory-index.md) 미구현) + tick-processed 발행.
10k/s에서 오히려 400/s로 떨어지는 건 GC/컨텍스트 스위칭 경합이다.

시사점:
- ADR-040의 T2 파티션 산정(150k/s ÷ 2,000 = 75 → 256)은 600/s 기준으로 **250 파티션이 최소**이고,
  그 전에 ADR-044(틱당 DB 조회 제거)가 선행되어야 파티션당 처리량 자체가 올라간다.
  파티션을 늘려도 컨슈머당 600/s면 250개 컨슈머 스레드가 필요하다.
- 캔들 flush 실패는 0 — 부하 중 정합성은 유지됐다(단 처리가 늦었을 뿐).

#### 측정 중 발견한 결함

- **`1c48b17` — worker에 HTTP 서버가 없었다.** `starter-actuator`만 있고 `starter-web`이 없어
  `server.port=8081`이 죽은 설정이었다. K8s의 worker 4종 readiness/liveness 프로브가 영원히 실패한다 —
  **운영 배포 시 liveness가 워커 pod를 주기적으로 죽였을 상황**이고, Prometheus도 워커를 못 긁어
  P1-2에서 계측한 메트릭이 전부 보이지 않았다. 워커 actuator를 읽으려다 발견했다.

### 5.5.1 심화 실험 M-001·M-002 (2026-09-17) — L-02·L-03을 더 깊이

L-02(ws-fanout)·L-03(tick-storm)의 기준선 위에서, 실시간 경로의 세 질문을 더 팠다. 각 조건 3회, p50/p95/p99.
전체 표·분석·재실행은 [reports/M-001](../reports/M-001.md)·[reports/M-002](../reports/M-002.md), 스크립트는
`bench/experiments/`. 여기서 바뀐 기본값은 [ADR-050](decisions/050-realtime-pipeline-defaults-from-load-tests.md)으로 묶었다.

**M-001 — WS push vs REST polling (N ∈ 1,000/5,000/10,000):**
- 같은 클라이언트 수에서 api CPU는 WS 0.1~0.5코어 vs 폴링 2.0~2.8코어(10~20배). 폴링은 ~5,300~5,500 req/s에서 천장을 쳐
  1만 명에게 0.5초 신선도(10만 req/s 필요)를 못 준다(95% drop) — **폴링은 실시간 갱신 수단이 못 된다**(D-M1-01, WS push 유지가 답).
- **한 api pod의 동시 WS 연결 상한 ≈ 8,192**(Tomcat `max-connections` 기본값, netstat로 확인). 연결당 힙 ~140KB.
  `max-connections`를 20,000으로 올리고 `-Xmx`와 같이 잡으면 N=10,000이 0 실패(D-M1-02).
- Kafka `docker kill` 30s → `start`: 모든 시세 45.7초 침묵(WS 연결 자체는 100% 유지), 유실 1,010건 = 게이트웨이 publish 실패와
  정확히 일치(acks 수정 후이므로 **인지된 유실**), worker·api 컨슈머는 재기동 없이 자동 재접속. **자동 폴백은 없지만 자동 복구는 된다.**
- 읽지 않는 WS 연결이 `clientOutboundChannel` 스레드(기본=코어 수)보다 많으면 **모든** 클라이언트가 p99 ~19초 멈춘다.
  send 한도(2s/256KB)로 폭주 세션을 끊고 outbound 풀을 50으로 넓히면 정상 p99 196ms(D-M1-04).

**M-002 — 파티션 P × 컨슈머 C 순서 보장 (202종목·2,015 tick/s 고정):**
- 12조건 모두 입력을 그대로 소화(처리량 병목 없음). 지연 최소는 **P6·C6**(p99 59~64ms), P=1은 파티션 하나라 C를 늘려도 288~1,578ms,
  P12·C6는 P6보다 못하거나 같다. **이 부하에선 파티션≈스레드≈6이 최적** — ADR-040의 로컬 12·운영 64는 "미래 틱 레이트"용 여유지 지금 성능값이 아니다.
- 키=stockId인데도 순서 위반이 났다(P>1 27회 중 9회) → 게이트웨이 `acks=0` 결함(D-M2-01). `RequireAll` 수정 후 키=stockId 45회 위반 0.
- worker 2프로세스 중 1개 SIGKILL: 유실·중복·위반 0이지만 그 파티션 45.0초 정지 + 생존자까지 17~18.5초 정지(D-M2-03).
  `session.timeout.ms=10000` + `CooperativeStickyAssignor`로 인계 10.6~12.5초·생존자 정지 ≤287ms.
- 핫 종목 처리비용이 스레드의 90%면 **같은 파티션 종목이 함께** p99 12.8~14.3초로 밀리고 다른 파티션은 무영향 — 키=stockId의 대가는 파티션이 운명 공동체라는 것.

**미해결로 남긴 것**: P6·C6에서도 3회 중 1회꼴로 p95 1.5~2.6초 꼬리 스파이크가 아무 조건에서나 났다. 분 경계 캔들 flush를 의심했으나
실행 창 교차 여부와 상관이 없어 **증명하지 못했다**(M-002 §4.4). §4.3 실시간 SLO(p99 300ms)를 P6C6도 안정적으로는 못 지킨다 — backlog §1로 분리.

**추가로 찾은 결함**: D-M2-02(게이트웨이가 발행 중 SIGTERM에 `Writer.Close`에서 무한 정지 — K8s 배포마다 grace period 소진 후 SIGKILL). 수정 후 75회 무재발.
...
"집계 손실이 없다"는 뜻이 아니다(§E4, 기록만). **주문 이벤트 컨슈머(quant-engine — 이후 [ADR-049](decisions/049-retire-quant-engine.md)로 폐기)는 로컬에서 띄우지 않았다** —
