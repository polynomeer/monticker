...
- [x] **홈 대시보드 연결** — ✅ 완료(2026-09-09, `7509891`). `PortfolioSnapshot`/`MarketSummary`/`RecentEvents`/`WatchlistSummary`/`TopMovers`(전부 [apps/web/src/components/home/](../apps/web/src/components/home/))가 이미 완성돼 실제 백엔드 API를 호출하도록 작성돼 있었으나 [app/page.tsx](../apps/web/src/app/page.tsx) 어디에서도 import되지 않는 고아 코드였음(도입 커밋 이후 한 번도 연결된 적 없음). 연결 과정에서 실제로 한 번도 실행되지 않았던 버그 3건 발견·수정: (1) `RecentEvents`/`TopMovers`가 `/stocks/{stockId}`(숫자 ID)로 링크했는데 `[symbol]` 라우트는 문자열 심볼만 매칭해 클릭 시 전부 404 — `/api/screener/quotes`로 stockId→symbol을 재해석하도록 수정(WatchlistTicker가 이미 쓰던 패턴과 통일). (2) `RecentEvents`/`TopMovers`/`WatchlistSummary` 셋이 각자 `/api/events/recent`를 따로 폴링해 한 화면에서 요청이 3중복 — 셋 다 동일한 react-query queryKey로 통일해 자동 합쳐지게 함. (3) `PortfolioSnapshot`의 하드코딩 hex 색상(`#ff5050`/`#4a8fd4`)을 `market-up`/`market-down` 토큰으로 교체, `TopMovers`엔 다크모드 색상 누락돼 있던 것도 같이 수정, `MarketSummary`는 WebSocket으로 잡히는 종목이 무제한 누적되던 것을 최근 12개로 제한. 라이브 검증: 로컬 인프라(monticker-postgres/redis/mongo) 복구 후 실제 API 기동, 로그아웃 상태에서 시장 위젯 정상 렌더링(개인화 위젯은 스스로 null 반환), 로그인 상태에서 실제 포트폴리오/관심종목 데이터 렌더링, "주목 종목" 클릭 시 실제 종목 상세 페이지로 정상 이동 확인. `tsc --noEmit`/lint/단위테스트(37/37) 전부 클린.
- [x] **"최근 본 종목" 기능 신설** — ✅ 완료(2026-09-09, `b57fa8f`). `useRecentlyViewedStocks` 훅([apps/web/src/hooks/useRecentlyViewedStocks.ts](../apps/web/src/hooks/useRecentlyViewedStocks.ts))이 서버 변경 없이 `localStorage`만으로 최근 10개 종목을 기록(로그인 불필요). 종목 상세 페이지([StockDetailClient.tsx](../apps/web/src/components/stock/StockDetailClient.tsx)) 마운트 시 기록, 노출 지점 2곳: 홈 화면 상단의 지울 수 있는 스트립([RecentlyViewedStocks.tsx](../apps/web/src/components/home/RecentlyViewedStocks.tsx)), 검색창([SearchAutocomplete.tsx](../apps/web/src/components/stock/SearchAutocomplete.tsx))에 포커스했을 때 검색어가 비어있으면 "최근 본 종목" 목록 표시(타이핑하면 기존 검색 결과로 전환). 라이브 검증: 실백엔드로 종목 2개 조회 후 홈/검색창 양쪽에 최신순으로 정확히 반영, 클릭 시 올바른 종목으로 이동 확인. 부수 발견(이번 스코프 밖으로 별도 플래그): `/watchlist` 등 오늘 건드리지 않은 페이지에서도 재현되는 앱 전역 React hydration 불일치 — `git stash`로 이번 변경과 무관함을 확인 후 별도 세션으로 분리(task_df405b3e).
- [x] **주문 사후관리 통합 뷰** — ✅ 완료(2026-09-09, `6f11463`). 조사해보니 [app/brokerage/orders/page.tsx](../apps/web/src/app/brokerage/orders/page.tsx)는 실제로는 주문 "제출" 폼일 뿐 내역 목록이 없었고, 진짜 주문 내역은 [app/brokerage/page.tsx](../apps/web/src/app/brokerage/page.tsx) 허브의 "주문내역" 탭에 있었음 — 문제의 실체는 그 탭이 일반 주문만 보여주고 조건부 주문([app/brokerage/conditional-orders/page.tsx](../apps/web/src/app/brokerage/conditional-orders/page.tsx))은 완전히 별도 페이지로 떨어져 있던 것. "주문내역" 탭이 이제 `useBrokerageOrders`+`useConditionalOrders`를 같은 페이지 인덱스로 함께 조회해 시간순으로 합치고 일반/조건부 배지로 구분해서 보여줌 — 두 페이지의 행 렌더링 로직을 [components/brokerage/{OrderRow,ConditionalOrderRow}.tsx](../apps/web/src/components/brokerage/)로 공유 추출해 중복·동작 불일치 없앰. 모의투자 체결([app/matching/page.tsx](../apps/web/src/app/matching/page.tsx))은 의도적으로 합치지 않음 — 실제 돈과 가상 돈을 한 타임라인에 섞는 건 벤치마크 보고서가 경고하는 유형의 혼동(IBKR가 GlobalTrader/Mobile을 제품 레벨로 분리한 사례 참고)이라 그대로 분리 유지. 라이브 검증: mock-social 테스트 유저 + MOCK 브로커리지 계좌로 일반 주문 2건(체결/접수)+조건부 주문 1건을 실제로 생성해 병합 뷰에서 정확한 시간순·배지로 표시됨을 확인, 병합 뷰에서 조건부 주문 취소 시 즉시 상태 반영까지 확인.
...
- [x] **차트 고급 모드** — ✅ 완료(구현 2026-09-10, 커밋 2026-09-21 `ae2a9bb`/`a50238b`). 자유 지표 추가(MA5/MA20/MA60/볼린저밴드 토글)·차트 위 주문선·드로잉 툴(추세선/수평선) 전부 구현 — 세 하위 기능 중 최소 범위(지표만) 대신 전부를 명시적으로 선택. 착수 전 조사에서 이 레포에 서로 단절된 주문 시스템 3개(모의투자 `paper`는 즉시체결만·미체결 개념 없음, `matching`은 미체결 있지만 종목 상세와 연결 안 된 별도 페이지, `brokerage`는 실브로커 API 왕복하는 진짜 미체결)가 있음을 발견 — 종목 상세에서 의미 있는 "미체결"을 가진 건 브로커리지뿐이라 주문선을 브로커리지 전용으로 한정, 드래그 수정은 브로커 API 왕복 지연·레이트리밋·부분체결 레이스 때문에 지원하지 않고 클릭 취소만 지원(수정하려면 어차피 취소 후 재주문이라 클릭 취소로 충분). 백엔드에 `GET /api/brokerage/orders/active?symbol=` 신설(종목 상세 화면 하나 분량이라 페이지네이션 없음). 드로잉은 ECharts `graphic` API로 구현(별도 캔버스/SVG 오버레이 불필요), 데이터 좌표(시각+가격)로 저장해 줌/팬해도 캔들에 고정, localStorage에 종목+interval별로 저장. 구현 중 실제 버그 발견·수정: `StockChart.tsx`의 `theme`와 `StockDetailClient.tsx`의 `orderLines`가 매 렌더 새 객체/배열 참조로 생성되고 있었는데 이게 차트 재생성 이펙트의 dependency라서 드로잉 한 점 찍을 때마다 차트 전체가 dispose+재생성되던 것 — `useMemo`로 고정해 해결. 라이브 검증: webpack 청크 레지스트리로 추출한 실 ECharts 인스턴스를 `getOption()`/`convertToPixel`/`convertFromPixel`로 직접 조회해 주문선 데이터·지표 시리즈 추가삭제·드로잉 두 종류의 배치와 좌표 변환·localStorage 영속·리사이즈 시 재배치까지 확인(픽셀 좌표 추측 스크린샷 방식 대신). `tsc`/lint/api 477건/worker 82건/web 37건 전부 클린.
...
- [~] **실시간 파이프라인 p95 꼬리 스파이크 원인 규명** — 규명 완료([reports/M-002-tail](../reports/M-002-tail.md), 2026-09-17). 두 종류였다:
  (A) 분 경계 캔들 flush 를 리스너 스레드에서 동기로 하던 것 → **수정**(배치 off-thread flush, [ADR-050](decisions/050-realtime-pipeline-defaults-from-load-tests.md) #4).
  (B) 간헐적(~40% 실행) JVM 전역 fetch 스톨 — GC·safepoint·flush·postgres·redis 전부 배제. 브로커 GC·CPU 를 계측해 재실험한 결과
  브로커 측 짧은 히컵(한 스파이크는 브로커 GC 93ms 와 겹침, 다른 스파이크는 미정렬; 브로커 CPU 는 간헐적으로 2코어 한도 195~233%에 붙음)이
  **CPU 포화 공유 호스트에서 수 초로 증폭**되는 것으로 판정 — 공유·축소 로컬 스택 아티팩트([reports/M-002-tail §2.3](../reports/M-002-tail.md)).
  **남은 일**: sub-second 방아쇠 확정은 아래 "전용 부하 테스트 환경"(§9, 전용 브로커·전용 CPU·RF≥2)에서. 페이퍼 규모·운영급 리소스에선 실사용 영향 없음.
...
- [x] **`PortfolioOptimizerService` 결과를 목표 비중에 바로 저장하는 편의 플로우** — [rebalance/page.tsx](../apps/web/src/app/brokerage/rebalance/page.tsx)에 "최적 비중 계산해 채우기" 버튼 추가(프론트 전용, 백엔드 무변경). 목표 행의 종목으로 `/api/analytics/portfolio/optimize` 호출 → **stockId 키 결과를 symbol 키 목표 비중으로 매핑**해 각 행에 채우고, `source`를 `OPTIMIZER`로 저장(기존엔 항상 `MANUAL`). 기대수익률·위험·제안 요약 표시, 수동 편집 시 `MANUAL`로 되돌림, 반올림 초과분(합 100%↑)은 최대 비중에서 차감. 저장된 목표(행에 id 없음)는 검색 API로 stockId 보강. **검증 한계**: dev 프리뷰가 fake 토큰이라 인증 엔드포인트가 500 — 최적화 클릭까지의 E2E는 재현 못함. tsc·웹 테스트 40건 통과, 버튼 렌더 확인, optimize↔target 키 계약은 소스로 확인.
