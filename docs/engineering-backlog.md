...
- [~] **실시간 파이프라인 p95 꼬리 스파이크 원인 규명** — 규명 완료([reports/M-002-tail](../reports/M-002-tail.md), 2026-09-17). 두 종류였다:
  ...
  (B) 간헐적(~40% 실행) JVM 전역 fetch 스톨 — GC·safepoint·flush 어느 것도 아니어서 **worker 밖(Kafka 단일 브로커/공유 호스트 CPU·IO)으로 좁혔다.**
  **남은 일**: (B)를 확정하려면 브로커 측 GC·CPU 계측 + 아래 "전용 부하 테스트 환경"(§9)이 필요하다. 페이퍼 규모에선 실사용 영향 없음.
