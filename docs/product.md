...
| AI 자동 매수/매도 | ✅ 모의투자 한정, 수동 요청 트리거로 1라운드 완료([ADR-036](decisions/036-ai-order-proposal.md)) — **가드레일 원칙대로**: LLM은 방향(BUY/SELL/HOLD)과 근거만 담은 주문 제안만 생성하고, 수량·가격은 절대 정하지 않는다. 승인은 제안 상태만 바꿀 뿐 주문을 제출하지 않고, 실제 제출은 사용자가 리스크 게이트가 있는 기존 주문 폼에서 직접 눌러야 한다 — "승인 즉시 자동 체결"로 새는 경로가 구조적으로 없다. "감정 태그 ≠ 투자 조언" 원칙과 동일한 선을 지킨다. 실브로커리지 확장·정기 스캔 자동 생성·투자자문업 라이선스 법률 검토는 범위 밖 |
