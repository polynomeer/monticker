...
function MetricCard({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="p-3 rounded-lg bg-[#282a36] border border-[#44475a]">
      <p className="text-xs text-[#6272a4] mb-1">{label}</p>
      <p className={`text-base font-bold ${highlight ? "text-[#bd93f9]" : "text-[#f8f8f2]"}`}>{value}</p>
    </div>
  );
}
...
export default function QuantLabDetailPage() {
  ...
  if (rsLoading) return <div className="p-8 text-[#6272a4]">로딩 중...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <button onClick={() => router.push("/quant-lab")} className="text-[#6272a4] hover:text-[#f8f8f2] text-sm mb-2 block">← 보관함</button>
          <h1 className="text-xl font-bold text-[#f8f8f2]">{ruleSet.name}</h1>
          {ruleSet.description && <p className="text-sm text-[#6272a4] mt-1">{ruleSet.description}</p>}
        </div>
        <button
          onClick={() => router.push(`/quant-lab/builder?edit=${id}`)}
          className="px-4 py-2 rounded-lg text-xs font-medium bg-[#44475a] text-[#f8f8f2] hover:bg-[#6272a4] transition-colors"
        >
          룰셋 수정
        </button>
      </div>
      ...
      {/* 백테스트 실행 패널 */}
      <div className="mb-8 p-5 rounded-xl border border-[#44475a] bg-[#21222c]">
        <h2 className="text-sm font-semibold text-[#f8f8f2] mb-4">백테스트 실행</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg bg-[#282a36] border border-[#44475a] text-[#f8f8f2] px-3 py-2 text-xs focus:outline-none focus:ring-2 focus:ring-[#bd93f9]"
            >
              {STOCKS.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[#6272a4] mb-1 block">시작일</label>
            <input
          ...
          <div>
            <label className="text-xs text-[#6272a4] mb-1 block">종료일</label>
            <input
          ...
          <div>
            <label className="text-xs text-[#6272a4] mb-1 block">초기 자본 (원)</label>
            <input
        ...
        <p className="mt-2 text-xs text-[#6272a4] text-center">
      ...
      {latestResult && (
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            <h2 className="text-sm font-semibold text-[#f8f8f2]">최신 백테스트 결과</h2>
            ...
            <span className="text-xs text-[#6272a4] ml-auto">
      ...
      {results.length > 1 && (
        <div>
          <h2 className="text-sm font-semibold text-[#f8f8f2] mb-3">이전 백테스트 이력</h2>
          <div className="space-y-2">
            {results.slice(1).map((r: BacktestResult) => (
              <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-[#21222c] border border-[#44475a] text-xs">
                <span className="text-[#6272a4]">{r.startDate} ~ {r.endDate}</span>
                <span className={`font-bold ${(r.totalReturn ?? 0) >= 0 ? "text-[#50fa7b]" : "text-[#ff5555]"}`}>
                  {fmt(r.totalReturn)}
                </span>
                <span className="text-[#6272a4]">MDD {fmt(r.mdd)}</span>
                {r.reliabilityScore && (
                  <span className={`font-bold ${RELIABILITY_COLOR[r.reliabilityScore]}`}>
                    {r.reliabilityScore}
                  </span>
                )}
                <span className="text-[#44475a]">{new Date(r.createdAt).toLocaleDateString("ko-KR")}</span>
              </div>
            ))}
          </div>
        </div>
