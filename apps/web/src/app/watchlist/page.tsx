...
export default function WatchlistPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6 dark:text-[#f8f8f2]">관심종목</h1>
      ...
      <form onSubmit={handleCreateGroup} className="flex gap-2 mb-8">
        ...
        <button
          type="submit"
          className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 dark:bg-[#bd93f9] dark:text-black dark:hover:bg-[#a87de8]"
        >
          그룹 추가
        </button>
      </form>
      ...
      {groups.length === 0 ? (
        ...
        <div className="space-y-6">
          {groups.map((group) => (
            <div key={group.id} className="border border-gray-200 dark:border-[#44475a] dark:bg-[#282a36] rounded-lg p-4">
              <h2 className="font-semibold text-lg mb-3 dark:text-[#f8f8f2]">{group.name}</h2>
