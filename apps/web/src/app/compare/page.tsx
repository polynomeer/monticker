...
function ComparePane({ symbol }: { symbol: string }) {
  ...
  if (loading || !stockId) return <div className="h-56 bg-gray-100 animate-pulse rounded-lg" />;

  return (
    <div>
      <p className="text-sm font-semibold text-gray-600 mb-1">{symbol}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {symbols.map(s => (
          <span key={s} className="flex items-center gap-1 bg-gray-100 px-3 py-1 rounded-full text-sm">
            {s}
            <button onClick={() => setSymbols(prev => prev.filter(x => x !== s))} className="text-gray-400 hover:text-gray-600 ml-1">×</button>
          </span>
        ))}
      </div>
