...
const PLAN_ACCENTS: Record<string, { border: string; badge: string; btn: string; icon: string }> = {
  FREE:  { border: "border-[#44475a]",      badge: "bg-[#44475a] text-[#f8f8f2]",        btn: "bg-[#44475a] text-[#f8f8f2] hover:bg-[#6272a4]",               icon: "🌱" },
...
function PlanCard({
  ...
  return (
    <div className={`relative flex flex-col p-6 rounded-2xl border bg-[#21222c] transition-all ${isCurrent ? accent.border + " ring-1 ring-[#bd93f9]/30" : accent.border}`}>
      {isCurrent && (
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <p className="text-xs text-[#6272a4]">{plan.name}</p>
          <p className="text-xl font-bold text-[#f8f8f2]">{won(plan.price)}</p>
        </div>
      </div>

      <ul className="flex-1 space-y-2 mb-6">
        {(plan.features ?? []).map((f, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-[#6272a4]">
      ...
      <button
        ...
        className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-colors disabled:opacity-40 ${accent.btn}`}
      >
...
export default function SubscriptionPage() {
  ...
  const STATUS_LABELS: Record<string, { label: string; color: string }> = {
    ...
    CANCELLED: { label: "해지", color: "text-[#6272a4]" },
  };
  ...
  const PAYMENT_LABELS: Record<string, { label: string; color: string }> = {
    ...
    REFUNDED: { label: "환불",      color: "text-[#6272a4]" },
  };

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-xl font-bold text-[#f8f8f2]">구독 플랜</h1>
        <p className="text-xs text-[#6272a4] mt-0.5">더 강력한 분석 도구로 업그레이드하세요</p>
      </div>

      {/* 현재 구독 상태 배너 */}
      {mySub && (
        <div className="mb-6 p-4 rounded-xl border border-[#44475a] bg-[#21222c] flex items-center justify-between gap-4">
          <div>
            <p className="text-xs text-[#6272a4]">현재 구독</p>
            <p className="font-semibold text-[#f8f8f2] mt-0.5">
              {mySub.planCode} 플랜{" "}
      ...
      {activeTab === "plans" && (
        plansLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[1, 2, 3].map(i => <div key={i} className="h-72 rounded-2xl bg-[#44475a]/20 animate-pulse" />)}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      {activeTab === "history" && (
        paymentsLoading ? (
          <div className="space-y-2">
            {[1,2,3].map(i => <div key={i} className="h-16 rounded-xl bg-[#44475a]/20 animate-pulse" />)}
          </div>
          ...
          <div className="space-y-2">
            {(paymentsData ?? []).map((p: PaymentRecord) => {
              const meta = PAYMENT_LABELS[p.status] ?? { label: p.status, color: "text-[#f8f8f2]" };
              return (
                <div key={p.id} className="flex items-center gap-3 p-4 rounded-xl border border-[#44475a] bg-[#21222c]">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-[#f8f8f2]">{p.planCode} 플랜</span>
