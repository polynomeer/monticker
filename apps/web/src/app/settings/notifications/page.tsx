...
function Toggle({ checked, onChange, label, description }: { checked: boolean; onChange: (v: boolean) => void; label: string; description?: string }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-[#44475a] last:border-0">
      <div>
        <p className="text-sm font-medium text-[#f8f8f2]">{label}</p>
        {description && <p className="text-xs text-[#6272a4] mt-0.5">{description}</p>}
      </div>
      <button
        role="switch"
        aria-checked={checked}
        onClick={() => onChange(!checked)}
        className={`relative w-11 h-6 rounded-full transition-colors ${checked ? "bg-[#bd93f9]" : "bg-[#44475a]"}`}
      >
...
export default function NotificationSettingsPage() {
  ...
  if (loading) return <div className="p-8 text-[#6272a4] text-sm">로딩 중...</div>;

  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      <h1 className="text-xl font-bold text-[#f8f8f2] mb-6">알림 설정</h1>

      <section className="mb-6 p-5 rounded-xl border border-[#44475a] bg-[#21222c]">
        <h2 className="text-sm font-semibold text-[#bd93f9] mb-3">전체 알림</h2>
      ...
      <section className="mb-6 p-5 rounded-xl border border-[#44475a] bg-[#21222c]">
        <h2 className="text-sm font-semibold text-[#50fa7b] mb-3">가격 알림</h2>
      ...
      <section className="mb-6 p-5 rounded-xl border border-[#44475a] bg-[#21222c]">
        <h2 className="text-sm font-semibold text-[#ffb86c] mb-3">뉴스·공시 알림</h2>
      ...
      <section className="mb-8 p-5 rounded-xl border border-[#44475a] bg-[#21222c]">
        <h2 className="text-sm font-semibold text-[#8be9fd] mb-3">리포트</h2>
