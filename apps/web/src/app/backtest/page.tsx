...
export default function BacktestPage() {
  ...
  const { data, isLoading, error } = useQuery<BacktestResult>({
    ...
    queryFn:  async () => {
      ...
      if (!res.ok) { const e = await res.json(); throw new Error(e.error ?? "백테스트 실패"); }
      return res.json();
    },
    enabled: !!submitted,
    staleTime: Infinity,
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold dark:text-[#f8f8f2]">백테스팅</h1>
      ...
      {/* 설정 패널 */}
      <div className="border dark:border-[#44475a] dark:bg-[#282a36] rounded-xl p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* 종목 */}
        <div>
          <label className="text-xs dark:text-[#6272a4] mb-1 block">종목</label>
          <select value={stockId} onChange={e => setStockId(Number(e.target.value))}
            className="w-full border dark:border-[#44475a] dark:bg-[#44475a] dark:text-[#f8f8f2] rounded-lg px-3 py-2 text-sm">
        ...
        {/* 전략 */}
        <div>
          <label className="text-xs dark:text-[#6272a4] mb-1 block">전략</label>
          <select value={strategy} onChange={e => setStrategy(e.target.value)}
            className="w-full border dark:border-[#44475a] dark:bg-[#44475a] dark:text-[#f8f8f2] rounded-lg px-3 py-2 text-sm">
            {STRATEGIES.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
          </select>
          <p className="text-[10px] dark:text-[#6272a4] mt-0.5">
        ...
        {/* 날짜 */}
        <div>
          <label className="text-xs dark:text-[#6272a4] mb-1 block">시작일</label>
          <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)}
            className="w-full border dark:border-[#44475a] dark:bg-[#44475a] dark:text-[#f8f8f2] rounded-lg px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="text-xs dark:text-[#6272a4] mb-1 block">종료일</label>
        ...
        {/* 초기 자본 */}
        <div>
          <label className="text-xs dark:text-[#6272a4] mb-1 block">초기 자본 (원)</label>
          <input type="number" value={capital} onChange={e => setCapital(Number(e.target.value))}
            step={1000000} className="w-full border dark:border-[#44475a] dark:bg-[#44475a] dark:text-[#f8f8f2] rounded-lg px-3 py-2 text-sm" />
        </div>

        {/* 손절/익절 */}
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="text-xs dark:text-[#6272a4] mb-1 block">손절 (%)</label>
            <input type="number" value={stopLoss} onChange={e => setStopLoss(Number(e.target.value))}
              step={1} min={1} max={50}
              className="w-full border dark:border-[#44475a] dark:bg-[#44475a] dark:text-[#f8f8f2] rounded-lg px-3 py-2 text-sm" />
          </div>
          <div className="flex-1">
            <label className="text-xs dark:text-[#6272a4] mb-1 block">익절 (%)</label>
        ...
        <div className="sm:col-span-2">
          <button onClick={handleRun} disabled={isLoading}
            className="w-full py-2.5 rounded-lg font-semibold text-sm bg-blue-600 dark:bg-[#bd93f9] text-white dark:text-[#282a36] hover:opacity-90 disabled:opacity-50">
